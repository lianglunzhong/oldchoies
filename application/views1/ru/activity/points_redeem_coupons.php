...
<head>
    ...
    <script src="/js/plugin.js"></script>
     <style type="text/css">
		...
		.points-box{
			background:url(/assets/images/en/1601/points-bg.jpg) repeat-y;
		}
		.points-on{
			position: absolute;
			background: #d06f37;
...
<body>
		<div class="site-content" id="phone-main">
			<div class="points-banner hidden-xs">
				<div class="container">
					<img  src="/assets/images/en/1601/points-banner-pc.jpg">
				</div>
			</div>
			<div class="points-banner-login hidden-xs">
				<div class="container">
					<img  src="/assets/images/en/1601/points-banner-pc-02.jpg">
					<?php
					//判断用户是否登录
					...
					<?php
						...
						{
					?>
						<div class="points-login">Please <a href="/customer/login?redirect=/activity/points_redeem_coupons">Sign In</a> to Redeem.</div>
					<?php 
						}else{
					?>
						<div class="points-login">Hi,<?php if($user_session['firstname']){ echo $user_session['firstname'];}else{ echo "choieser";}?>! You have <span class="red" id="ajaxpoints"><?php echo $data['user_points'];?></span> activated points to use.</div>
					<?php
						} 
					?>
				</div>		
			...
			<div class="points-banner-login-mobile  hidden-sm hidden-md hidden-lg">
				<div class="container" >
					<img  src="/assets/images/en/1601/points-banner-mobile.jpg">
					<?php
						if(!$user_session['id'])
						{
					?>
						<div class="points-login-mobile" style="left:30%;">Please <a href="/customer/login?redirect=/activity/points_redeem_coupons">Sign In</a> to Redeem.</div>
					<?php 
						}else{
					?>
						<div class="points-login-mobile" style="left:28%;"><span class="red" id="ajaxpoints_m"><?php echo $data['user_points'];?></span> Points to Use</div>
					<?php
						} 
					?>
				</div>		
				...
				<div class="container">
					...
					?>
						<li class="col-xs-12 col-sm-6 col-md-4">
							<a href="javascript:void(0)" id="code" class="exchange"  >
								<img  src="/assets/images/en/<?php echo $value['imgurl']; ?>">
								<?php
								if($data['user_points'] < $value['value'])
								{
								?>
									<div class="points-off" id="js_<?php echo $value['value'];?>">NOT ENOUGH POINTS</div>
								<?php
								}else{
								?>
									<a href="javascript:void(0);" class="points-on" data-reveal-id="myModal" data-value="<?php echo $value['value'];?>" data-name="<?php echo $value['name'];?>" id="js_<?php echo $value['value'];?>">REDEEM NOW</a>
								<?php
								}
								?>
					...
					?>
						<li class="col-xs-12 col-md-6 hidden-xs">
							<a href="javascript:void(0)" class="exchange-pc" data-value="<?php echo $value['value']; ?>" data-name="<?php echo $value['name'];?>">
								<img  src="<?php echo LANGPATH; ?>/assets/images/en/<?php echo $value['imgurl']; ?>" >
								<?php
								if($data['user_points'] < $value['value'])
								{
								?>
					...
					?>
						<li class="col-xs-12 col-sm-6 hidden-md hidden-lg">
							<a href="javascript:void(0)" class="exchange-m" data-value="<?php echo $value['value']; ?>" data-name="<?php echo $value['name'];?>">
								...
								{
								?>
									<div class="points-off" id="jsm_<?php echo $value['value'];?>">NOT ENOUGH POINTS</div>
								<?php
								}else{
								?>
									<a href="javascript:void(0);" class="points-on" data-reveal-id="myModal" data-value="<?php echo $value['value'];?>" data-name="<?php echo $value['name'];?>" id="jsm_<?php echo $value['value'];?>">REDEEM NOW</a>
								<?php
								}
								?>
							</a>
					...
					<div class="points-notes">
						<p>
							...
							1.By <strong>Feb.18th, 2016</strong>, all points will be reset. Please use them to redeem coupons as soon as possible.<br>
							2.The coupon codes will be valid till <strong>May 31st, 2016</strong>.<br>
							3. Once you have redeemed the points, they won’t be returned to your account. <br>
							4. Activity time: <strong>Ends Feb. 17th EST</strong>.<br>
							5. Our cooperative bloggers/partners are excluded. You can keep your points.<br><br>
							Choies reserves the right of final interpretation of this activity.
						</p>
					</div>
...
<div id="myModal" class="reveal-modal large">
        ...
        <div class="confirm" >
        	<p class="text">Are you sure to redeem the points?</p>
        	<p class="button">
        	  <input type="hidden" value="" id="but1">
        	  <input type="hidden" value="" id="but2">
		      <button type="button" class="btn btn-primary btn-sm" id="point_confirm">Confirm</button>
		      <button type="button" class="btn btn-default btn-sm close-reveal-modal" style="font-size:12px; 
		      position: initial; line-height:26px; color:#fff;">Cancel</button>
		    </p>
        </div>
        <div class="thanks" style="display:none;">
        	<p>Thanks! Successfully Redeemed!</p>
        	<p class="blod he" id="thinkspoints">REDAS12153413</p>
         	<p class="red he">（Expires on May 31st, 2016.）</p>
           	<p>Check It in <a href="<?php echo LANGPATH; ?>/customer/coupons" target="_blank"> My Coupons ></a></p>
        	<p>^_^ We will also send you an email.</p>
        </div>
    </div>
		    ...
		    $('#point_confirm').live("click",function(){
	            ...
	            success:function(res){
	            	if(res['success']==1)
	            	{
	        			...
	        			{
							$("#js_"+res['all'][p]).removeClass().addClass("points-off").removeAttr("data-reveal-id").html("NOT ENOUGH POINTS");
							//.手机端m
							$("#jsm_"+res['all'][p]).removeClass().addClass("points-off").removeAttr("data-reveal-id").html("NOT ENOUGH POINTS");
						}
						for(var p in res['allok'])
	        			{
							$("#js_"+res['all'][p]).removeClass().addClass("points-on").attr("data-reveal-id","myModal").html("REDEEM NOW");
							//手机端m
							$("#jsm_"+res['all'][p]).removeClass().addClass("points-on").attr("data-reveal-id","myModal").html("REDEEM NOW");
						}
				...
				$.ajax({
	                ...
	                success:function(res){
	                	if(res['success']==1)
	                	{
                			...
                			for(var p in res['all'])
                			{
								$("#js_"+res['all'][p]).removeClass().addClass("points-off").html("NOT ENOUGH POINTS");
							}
							for(var p in res['allok'])
                			{
								$("#js_"+res['all'][p]).removeClass().addClass("points-on").html("REDEEM NOW");
							}
	                	}
	                }
            	})
