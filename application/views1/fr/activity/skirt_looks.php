            ...
            <div class="pro-list">
                <ul class="row skirt">
                    ...
                    <li class="pro-item col-xs-6 col-sm-3">
                        ...
                        <div class="ski"><a class="vi" target="_blank" href="<?php echo $value['product_url'];?>">Voir Détails >></a><a data-reveal-id="myModal" class="flr btn btn-default btn-xs hidden-xs" title="<?php echo $value['link_sku'];?>" onclick="load_data('<?php echo $value['link_sku'];?>')" >Styles et Conseils</a></div>
                    </li>
                    <?php } ?>
                </ul>
                  <div class="skirtban" style="display:none;"><a target="_blank" href="<?php echo LANGPATH; ?>/skirt-c-99"><img src="<?php echo STATICURL; ?>/ximg/activity/skirt_looks/skirt-08.jpg" /></a></div>                 
            </div>
        </div>
    </div>
</div>
    ...
    $.post(
        ...
        function (data) {
            ...
            for (var i = item.length - 1; i >= 0; i--) {
                ...
                str += '<li><input type="checkbox" name="check['+item[i]['n']+']" title="size'+item[i]['n']+'" class="checkbox" checked="checked" id="checkout'+item[i]['product_id']+'" /> <label for="checkout'+item[i]['product_id']+'">Ajouter au panier</label><input type="hidden" name="item['+item[i]['n']+']" value="'+item[i]['product_id']+'" /><a href="'+item[i]['sku_link']+'"><img src="'+item[i]['img_src']+'" /></a><a href="'+item[i]['sku_link']+'" class="name">'+item[i]['product_name']+'</a><p class="price">'+price+'</p>'+item[i]['options']+'<p class="center"><a href="'+item[i]['sku_link']+'" class="btn22_gray" target="_blank">Voir Détails</a></p></li>';
            };
...
<div id="myModal" class="reveal-modal xlarge">
    ...
    <div class="look-pro">
        <form action="<?php echo LANGPATH; ?>/cart/add_more" method="post" class="form3" id="form">
            ...
            <div class="add-bag">
                <input value="ADD TO BAG" class="btn btn-primary btn-lg" type="submit"><a href="<?php echo LANGPATH; ?>/wishlist/add_more/" class="a-underline add-wishlist">Ajouter à la liste d'envies</a>
            </div>
        </form>
        <script>
            $("#form").validate({
