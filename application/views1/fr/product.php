                        ...
                        <div id="JS_productPic" class="productpic loadding">
                           ...
                           if($show_ship_tip){
                            ...
                            if (!($data = $cache->get($key))){
                                ...
                                $ready_shippeds = DB::select('product_id')->from('products_categoryproduct')->where('catalog_id', '=', 395)->where('deleted','=',0)->execute()->as_array();
                ...
                <div class="box-dibu1">
                    <div>
                        ...
                        <div class="hide-box-1 hide">
                            ...
                            {
                                $news = DB::query(Database::SELECT, 'SELECT id, name, link, price, has_pick FROM products_product WHERE set_id = ' . $product->get('set_id') . ' AND visibility = 1 AND status=1 AND stock <> 0 ORDER BY display_date DESC LIMIT 0, 7')->execute()->as_array();
                        ...
                        <div class="hide-box-2 hide">
                            ...
                            {
                            $hots = DB::query(Database::SELECT, 'SELECT cp.product_id FROM products_categoryproduct cp left join products_product p on  cp.deleted = 0 AND p.deleted = 0 AND cp.product_id = p.id   WHERE cp.catalog_id = 32 and p.visibility = 1 AND p.status=1 AND p.stock <> 0 ORDER BY cp.position DESC LIMIT 0, 7')->execute()->as_array();
