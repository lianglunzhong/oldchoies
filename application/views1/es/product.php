                ...
                <div class="box-dibu1">
                    <div>
                        ...
                        <div class="hide-box-2 hide">
                            ...
                            {
                            $hots = DB::query(Database::SELECT, 'SELECT cp.product_id FROM products_categoryproduct cp left join products_product p on cp.product_id = p.id   WHERE cp.deleted=0 AND p.deleted=0 AND cp.category_id = 32 and p.visibility = 1 AND p.status=1 AND p.stock <> 0 ORDER BY cp.position DESC LIMIT 0, 7')->execute()->as_array();
