...
<head>
    ...
    <script src="/js/plugin.js"></script>
     <style type="text/css">
		...
		.points-box{
			background:url(/assets/images/es/1601/points-bg.jpg) repeat-y;
		}
		.points-on{
			position: absolute;
			background: #d06f37;
		...
		<div class="site-content" id="phone-main">
			...
			<div class="points-banner-login hidden-xs">
				<div class="container">
					...
					<?php
						...
						{
					?>
						<div class="points-login">Por favor<a href="/es/customer/login?redirect=/es/activity/points_redeem_coupons">ingrese</a> para canjear.</div>
					<?php 
						}else{
					?>
						<div class="points-login">¡Hola <?php if($user_session['firstname']){ echo $user_session['firstname'];}else{ echo "choieser";}?>! Tiene <span class="red" id="ajaxpoints"><?php echo $data['user_points'];?></span> puntos activados sin usar.</div>
					<?php
						} 
					?>
				</div>		
			...
			<div class="points-banner-login-mobile  hidden-sm hidden-md hidden-lg">
				<div class="container" >
					...
					?>
						<div class="points-login-mobile" style="left:40%;"><span class="red" id="ajaxpoints_m"><?php echo $data['user_points'];?></span> Puntos para usar</div>
					<?php
						} 
					?>
				</div>		
				...
				<div class="container">
					...
					?>
						<li class="col-xs-12 col-sm-6 col-md-4">
							<a href="javascript:void(0)" id="code" class="exchange"  >
								...
								{
								?>
									<div class="points-off" id="js_<?php echo $value['value'];?>">PUNTOS NO SON SUFICIENTES</div>
								<?php
								}else{
								?>
									<a href="javascript:void(0);" class="points-on" data-reveal-id="myModal" data-value="<?php echo $value['value'];?>" data-name="<?php echo $value['name'];?>" id="js_<?php echo $value['value'];?>">Canjear Ahora</a>
								<?php
								}
								?>
					...
					?>
						<li class="col-xs-12 col-sm-6 hidden-md hidden-lg">
							<a href="javascript:void(0)" class="exchange-m" data-value="<?php echo $value['value']; ?>" data-name="<?php echo $value['name'];?>">
								...
								{
								?>
									<div class="points-off" id="jsm_<?php echo $value['value'];?>">PUNTOS NO SON SUFICIENTES</div>
								<?php
								}else{
								?>
									<a href="javascript:void(0);" class="points-on" data-reveal-id="myModal" data-value="<?php echo $value['value'];?>" data-name="<?php echo $value['name'];?>" id="jsm_<?php echo $value['value'];?>">Canjear Ahora</a>
								<?php
								}
								?>
							</a>
					...
					<div class="points-notes">
						<p>
							...
							1. Todos los puntos se restablecerán para el <strong>20 de Febrero de2016</strong>, por favor los utilice para canjear cupónes tan pronto como sea posible.<br>
							2. El código de cupón será válido hasta el <strong>31 de Mayo de 2016</strong>.<br>
							3. Una vez que canjee los puntos, no serán devueltos a su cuenta. <br>
							4. Tiempo de actividad: <strong>Finaliza el 20 de Febrero </strong>.<br>
							5. Quedan excluidos nuestros bloggers/compañeros de cooperación. Puede quedarse los puntos.<br><br>
							Choies se reserva el derecho a dar una interpretación final de esta actividad.
						</p>
					</div>
					<div class="other-customers">
						<div class="w-tit">
							<h2 style="background-color:#faf0e2;">Los bestsellers se recomiendan altamente</h2>
						</div>
						<div class="box-dibu1">
...
<div id="myModal" class="reveal-modal large">
        ...
        <div class="confirm" >
        	<p class="text">¿Está seguro de que desea canjear los puntos?</p>
        	<p class="button">
        	  <input type="hidden" value="" id="but1">
        	  <input type="hidden" value="" id="but2">
		      <button type="button" class="btn btn-primary btn-sm" id="point_confirm">CONFIRMAR</button>
		      <button type="button" class="btn btn-default btn-sm close-reveal-modal" style="font-size:12px; 
		      position: initial; line-height:26px; color:#fff;">CANCELAR</button>
		    </p>
        </div>
        <div class="thanks" style="display:none;">
        	<p>Muchas gracias,  ¡Éxito en canjear puntos!</p>
        	<p class="blod he" id="thinkspoints">REDAS12153413</p>
         	<p class="red he">（Expira el 31 de Mayo de 2016.）</p>
           	<p>Verlo en <a href="<?php echo LANGPATH; ?>/customer/coupons" target="_blank"> Mis Cupónes ></a></p>
        	<p>^_^ Vamos a enviar un correo electrónico a usted.</p>
        </div>
    </div>
		    ...
		    $('#point_confirm').live("click",function(){
	            ...
	            success:function(res){
	            	if(res['success']==1)
	            	{
	        			...
	        			{
							$("#js_"+res['all'][p]).removeClass().addClass("points-off").removeAttr("data-reveal-id").html("PUNTOS NO SON SUFICIENTES");
							//.手机端m
							$("#jsm_"+res['all'][p]).removeClass().addClass("points-off").removeAttr("data-reveal-id").html("PUNTOS NO SON SUFICIENTES");
						}
						for(var p in res['allok'])
	        			{
							$("#js_"+res['all'][p]).removeClass().addClass("points-on").attr("data-reveal-id","myModal").html("Canjear Ahora");
							//手机端m
							$("#jsm_"+res['all'][p]).removeClass().addClass("points-on").attr("data-reveal-id","myModal").html("Canjear Ahora");
						}
				...
				$.ajax({
	                ...
	                success:function(res){
	                	if(res['success']==1)
	                	{
                			...
                			for(var p in res['all'])
                			{
								$("#js_"+res['all'][p]).removeClass().addClass("points-off").html("PUNTOS NO SON SUFICIENTES");
							}
							for(var p in res['allok'])
                			{
								$("#js_"+res['all'][p]).removeClass().addClass("points-on").html("Canjear Ahora");
							}
	                	}
	                }
            	})
