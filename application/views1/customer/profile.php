...
if(empty(LANGUAGE))
{
	$lists = Kohana::config('/customer/account_setting.en');
}
else
{
	$lists = Kohana::config('/customer/account_setting.'.LANGUAGE);
}
?>
<section id="main">
	<!-- crumbs -->
	<div class="container">
		<div class="crumbs">
			<div>
				...
				<a href="<?php echo LANGPATH; ?>/customer/summary" class="visible-xs-inline hidden-sm hidden-md hidden-lg"> ><?php echo $lists['title2']; ?></a> ><?php echo $lists['title3']; ?> 			</div>
			...
			<article class="user col-sm-9 col-xs-12">
				...
				<div class="row">
					...
					<form action="#" method="post" class="user-form form col-sm-8" id="accountSettings">
						<ul>
							<li>
								<label class="col-sm-3 col-xs-12"><span>*</span> <?php echo $lists['Email']; ?></label>
                                ...
                                if ($customer['country'])
                            {
                                ...
                                $country_name = DB::select('name')->from('core_country')->where('isocode', '=', $customer['country'])->execute()->get('name');
								 echo $country_name;
                            }
                            else
                            {
							...
							<li>
								<label class="col-sm-3 col-xs-12"><?php echo $lists['Gender']; ?></label>
								<div class="col-sm-9 col-xs-12">
									<span class="w-radio"><input type="radio" name="gender" value="1" id="radio1" <?php if ($customer['gender'] == 1) echo 'checked'; ?> /> <?php echo $lists['Female']; ?></span>
									<span class="w-radio"><input type="radio" name="gender" value="0" id="radio2" <?php if ($customer['gender'] == 0) echo 'checked'; ?> /> <?php echo $lists['Male']; ?></span>
									<span class="w-radio"><input type="radio" name="gender" value="2" id="radio3" <?php if ($customer['gender'] == 2) echo 'checked'; ?> /> <?php echo $lists['Other']; ?></span>
								</div>
							</li>
							<li>								

								<label class="col-sm-3 col-xs-12"><?php echo $lists['Birthday']; ?></label>
								<div class="col-sm-9 col-xs-12">
