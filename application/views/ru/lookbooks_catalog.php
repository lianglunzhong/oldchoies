...
<section id="main">
    ...
    <div class="layout">
        <div class="crumbs fix">
            <div class="fll"><a href="<?php echo LANGPATH; ?>/">Home</a>  >  Лукбук</div>
        </div>
    </div>
    <section class="list_new layout fix">
        <h3 class="lookbook_tit">Лукбук</h3>
        <!-- lookbook_list -->
        <ul id="brand_waterfall" class="lookbook">
            <?php
            for ($i = 0; $i < 20; $i++)
            {
                if (isset($lookbooks[$i]['product_id']))
                {
                    $product_ins = Product::instance($lookbooks[$i]['product_id'], LANGUAGE);
                    if($product_ins->get('set_id') == 502)
                        continue;
                    $c = DB::select('id', 'image')->from('celebrity_images')->where('product_id', '=', $lookbooks[$i]['product_id'])->order_by('position')->execute()->current();
                    $lookbook['id'] = $c['id'] . '-' . '1';
...
<!--                        <div class="cell_like"><span href="#" class="icon_like">234</span></div>-->
                    </div>
                    <p class="name">
...
<script type="text/javascript">
        var page = <?php echo ceil($count / 20); ?>;
        var count = <?php echo $count; ?>;
        var opt={
                getResource:function(index,render){//index为已加载次数,render为渲染接口函数,接受一个dom集合或jquery对象作为参数。通过ajax等异步方法得到的数据可以传入该接口进行渲染，如 render(elem)
                        if(index > page) return false;
                        $.post(
                        '/site/lookbook_ajax?lang=<?php echo LANGUAGE; ?>',
                        {
                                index: index
                        },
                        function(lookbooks){
                                ...
                                for(var i=0;i<21;i++)
                                {
                                        ...
                                        if(i < 20)
                                        {
                                                ...
                                                html+='<li class="cell"><div class="cell_img">\n\
                                                <a target="_blank" href="<?php echo LANGPATH; ?>/lookbook/'+lookbooks[j]['id']+'"><img src="'+src+'" /></a></div>\n\
                                                <p class="name"><a target="_blank" href="<?php echo LANGPATH; ?>/lookbook/'+lookbooks[j]['id']+'">'+lookbooks[j]['title']+'</p></a></li>';
                                        }
                                }
                                for(var i=0;i<10;i++)
                                {
