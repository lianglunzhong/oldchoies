...
<section id="main">
	...
	<img src="/images/activity/skirt_looks/ru_skirt_look.jpg" border="0" usemap="#Map" />
            <map name="Map" id="Map">
<area shape="rect" coords="1,0,512,299" href="http://www.choies.com/ru/skirt?sort=1" />

<area shape="rect" coords="516,2,1022,299" href="http://www.choies.com/ru/skirt?sort=2" />
            </map>
    </div>
    ...
    <div class="layout">
        <ul class="skirt">
            ...
            <li>
                ...
                <div class="ski"><a class="vi" target="_blank" href="<?php echo $value['product_url'];?>">Деталь >></a><span class="ge JS_popwinbtn4" title="<?php echo $value['link_sku'];?>" onclick="load_data('<?php echo $value['link_sku'];?>')" >Другие В Фото</span></div>
            </li>
            <?php } ?>
        </ul>
...
function load_data(skus){
    $.post(
        '/activity/ajax_skirt_looks?lang=ru',
        {
            skus:skus,
        },
        function (data) {
            ...
            for (var i = item.length - 1; i >= 0; i--) {
                ...
                str += '<li><input type="checkbox" name="check['+item[i]['n']+']" title="size'+item[i]['n']+'" class="checkbox" checked="checked" id="checkout'+item[i]['product_id']+'" /> <label for="checkout'+item[i]['product_id']+'">Добавить</label><input type="hidden" name="item['+item[i]['n']+']" value="'+item[i]['product_id']+'" /><a href="'+item[i]['sku_link']+'"  target="_blank"><img src="'+item[i]['img_src']+'" /></a><a href="'+item[i]['sku_link']+'" class="name">'+item[i]['product_name']+'</a><p class="price">'+price+'</p>'+item[i]['options']+'<p class="center"><a href="'+item[i]['sku_link']+'" class="btn22_black" target="_blank">Детали</a></p></li>';
            };
    ...
    <div class="look_pro">
        <form action="<?php echo LANGPATH; ?>/cart/add_more" method="post" class="form3" id="form">
            ...
            <div class="center mt50">
                <input type="submit" value="Добавить в корзину" class="btn40_16_red" /><a href="" class="a_underline add_wishlist">Избранное</a>
            </div>
        </form>
        <script>
            $("#form").validate({
