    ...
    function AddNewRow() {
        ...
        for (var i = 0; i < columnLength; i++) {
            if (i == 0) {//第一列:序号
                ...
                newTR.insertCell(i).innerHTML = "<label><span class='red mb10'>*</span> Type du site principal: </label><select id='type_"+i+"' name='sites[type][]'><option value='Look Book'>Look Book</option><option value='Personal Blog'>Blog personnel</option><option value='Facebook'>Facebook</option><option value='Youtube'>Youtube</option><option value='Chictopia'>Chictopia</option><option value='Pinterest'>Pinterest</option><option value='Instagram'>Instagram</option><option value='Tumblr'>Tumblr</option><option value='Twitter'>Twitter</option></select><br><label><span class='red'>*</span> URL du site principal: </label><input type='text' id='url_"+i+"' name='sites[url][]' class='sub_info_text' style='margin-right:3px;' /><label><span class='red'>*</span> Fans: </label><input type='text' id='flow_"+i+"' name='sites[flow][]' class='sub_info_text2 mb10' style='width:120px; margin-left:5px;' />";
            }
            else if (i >= 2) {
                newTR.insertCell(i).innerHTML = "<label><span class='red'>*</span> URL du site principal: </label><input type='text' id='url_"+i+"' name='sites[url][]' class='sub_info_text' /><label><span class='red ml10'>*</span> Fans: </label><input type='text' id='flow_"+i+"' name='sites[flow][]' class='sub_info_text2' />";
            }
        }
        //添加列:删除按钮
        var lastTd = newTR.insertCell(columnLength);
        lastTd.innerHTML = "<div align='left' style='width:40px;'><a href='javascript:' onclick=\"DeleteSignRow('SignItem" + rowID + "')\">supprimer</a></div>";
        //将行号推进下一行
            ...
            if(email && gender && country)
            {
                ...
                function(data)
                {
                    if(data)
                    {
                        ...
                        $('#catalog_link').show();
                        var message = '<h3>Désolé!</h3><p>Cette adresse é-mail a été utilisée.</p>';
                    ...
                    else
                    {
                        ...
                        $('#catalog_link').appendTo('body').fadeIn(320);
                        $('#catalog_link').show();
                        var message = '<h3>Merci beaucoup!</h3><p>Nous vous contacterons dans une semaine après l\'envoi du formulaire de demande.</p>';
                        $('.fashion_thank').html(message);
                        setTimeout(function() {  
                            $("#submitInfo").submit();  
                        }, 3000);
...
<section id="main">
    ...
    <section class="layout fix">
      ...
      <span>Programme de Fashion</span><span>Lire les politiques</span><span class="on">Soumettre les informations</span><span>Obtenir un banner</span>
      <img src="/images/<?php echo LANGUAGE; ?>/blogger_wanted3.png" />
    </div>  
        <?php echo Message::get(); ?>
        <article style="margin-top:30px;background: #fff;">
            <div class="fashion_tit" style="margin-left:39px">SOUMETTRE LES INFORMATIONS</div>
            <div class="fll" style="padding-left:70px;">
                <div class="sub_info">
                    <form action="" method="post" id="submitInfo" class="form">
                        <ul>
                            <li>
                                <label style="margin-left:38px"><span class="red">*</span> Adresse é-mail:</label>
                                <input type="text" id="email" name="email" class="sub_info_text" />
                                <span class="errorInfo"></span>
                            </li>
                            <li>
                                <label style="margin-left:38px"><span class="red">*</span> Sexe:</label>
                            ...
                            <li>
                                <label style="margin-left:38px"><span class="red">*</span> Pays:</label>
                                ...
                                <div style="overflow: auto; width:100%; padding:0 10px; float:left">
                                    <table cellpadding="1" id="tab" cellspacing="0">
                                        <tr>
                                            ...
                                            <td>
                                                <label><span class="red mb10">*</span> Type du site principal: </label>
                                                <select maxlength="16" id="type" name="sites[type][]">
                                                    <option value="lookbook">Look Book</option>
                                                    <option value="personal Blog">Blog personnel</option>
                                                ...
                                                <br/><label><span class='red'>*</span> URL du site principal: </label>
                                                <input type='text' id='url' name='sites[url][]' class='sub_info_text' />
                                                <label><span class='red'>*</span> Fans: </label>
                                    ...
                                    <table cellpadding="1" cellspacing="0" style="text-align: center; display:block;">
                                        <tr>
                                            <td>
                                                <input type="button" name="Submit" class="add_more" style=" margin-left:139px;" value="Ajouter un autre site" onclick="AddNewRow()" />
                                            </td>
                                            <td>
                            ...
                            <li>
                                <label  style="margin-left:39px"><span class="red">*</span> Message:</label>
                                <textarea name="comment" id="input_comment" rows="10" class="input textarea sub_info_text3" onblur="this.value=(this.value=='')?'Écrire quelque chose pour vous présenter et votre opinion sur la fashion. Merci beaucoup!':this.value" value="Écrire quelque chose pour vous présenter et votre opinion sur la fashion. Merci beaucoup!" onfocus="this.value=(this.value=='Écrire quelque chose pour vous présenter et votre opinion sur la fashion. Merci beaucoup!')?'':this.value">Écrire quelque chose pour vous présenter et votre opinion sur la fashion. Merci beaucoup!</textarea>
                            </li>
                            <li>
                                <input type="submit" id="inforSubmit" class="form_btn view_btn btn30_14_red" style="width: 100px;height: 38px;" value="SOUMETTRE"/>
                            </li>
                        </ul>
                        <script type="text/javascript">
                            $("#submitInfo").validate({
