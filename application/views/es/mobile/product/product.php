                ...
                for(var p in product)
                {
                    if(key == 0)
                    {
                        cart_view = '<h2>success! item added to bag</h2>'
                            ...
                            +   '<ul><li>artículo#: '+product[p]['sku']+'<br/>'+product[p]['price']+'<br/>'+product[p]['attributes']+'</li>'
    ...
    <div class="banner-list">
        <ul class="fix" style="height:350px;">
            ...
            foreach ($product->images() as $image): $i++
                ?>
                <li><a href="#"><img src="<?php echo Image::link($image, 5); ?>" alt="<?php echo $product->get('name'); ?>" width="300" height="196"/></a></li>
                <?php
            endforeach;
            ?>
        </ul>
...
<div id="product-main">
    <header>
        ...
        <span style="margin-left:14px;">artículo# : <?php echo $product->get('sku'); ?></span><br/>
        <?php
        if ($p_price > $price)
        {
            ?>
