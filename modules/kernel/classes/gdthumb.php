...
class GdThumb extends ThumbBase
{
    ...
    public function resize ($maxWidth = 0, $maxHeight = 0)
    {
        ...
        $this->preserveAlpha();		
        //imageantialias($this->workingImage, true);
    ...
    public function save ($fileName, $format = null)
    {
        ...
        //锐化--begin
        $this->SHARP=0.4;//值越大就越锐化
        $cnt=0;
        for ($x=1; $x<$this->newDimensions['newWidth']; $x+=2){
            for ($y=1; $y<$this->newDimensions['newHeight']; $y+=2)
            {
                $src_clr1 = imagecolorsforindex($this->oldImage, imagecolorat($this->oldImage, $x-1, $y-1));
                $src_clr2 = imagecolorsforindex($this->oldImage, imagecolorat($this->oldImage, $x, $y));
                ...
                $g = min(255, max($g, 0));
                $b = min(255, max($b, 0));
                if (($DST_CLR=imagecolorexact($this->oldImage, $r, $g, $b))==-1)
                    $DST_CLR = imagecolorallocate($this->oldImage, $r, $g, $b);
                $cnt++;
                if ($DST_CLR==-1) die("color allocate faile at $x, $y ($cnt).");
                imagesetpixel($this->oldImage, $x, $y, $DST_CLR);
            }
        }
        //锐化--end**/
