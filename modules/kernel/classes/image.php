...
class Image
{
    ...
    public static function link($image, $size)
    {
        ...
        if( ! is_array($image) OR isset($image['id']) == '')
        {
       ...
       // {
       //         return BASEURL.'/pimages/'.$image['id'].'/'.$size.'.'.$image['suffix'];
       // }
       // else
       // {
       //         return BASEURL.'/pimages/'.$image['id'].'/'.$size.'.'.$image['suffix'];
       // }
        ...
        // $img_url = DB::select('image')->from('products_productimage')
        //                                 ->where('id','=',$image['id'])
        //                                 ->execute()->get('image');
        // if(!$img_url)
        // {
        //     $image_name = $image['id'].'.jpg';
        //     $img_url = 'pimgs/pimages/'.$image_name;
        // }
        // return LOCALURL.$img_url;
    ...
    public static function delete_uploads($id)
    {
        ...
        if($image->loaded())
        {
            ...
            $image->delete($id);
            Session::instance()->set('b2b_sync_sku', Product::instance($image->product_id)->get('sku'));
            return TRUE;
        }
        return FALSE;
    }
    ...
    public static function clear_cache($id)
    {
        ...
        $sizes = kohana::config('sites.thumbnail_sizes');
        $sizes[99] = array( 120, 120 );
        $pimages_dir = kohana::config('upload.resource_dir').DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR.'pimages';
