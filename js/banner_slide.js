...
var slide = (function() {
	...
	**/
	...
	var buildSlide = function() {
		...
		$('_slide').getElementsByTagName('ul')[0].style.left = '-' + aLi[0].offsetWidth * (config.imgDataLen-1) + 'px';
	};
	...
	var run = function(e, t){
		...
		config.curImg = config.index + 1 > (config.imgDataLen + 1) ? 1 : (config.index + 1);
		slideList[config.curImg].className = 'cur';
        var dotN = 0;
        if(config.index >= (config.imgDataLen - 1)){
	        dotN = config.index - (config.imgDataLen - 1);
        }else{
	        dotN = config.curImg;
        }
	...
	var autoSwitch = function() {
		...
		var autoFun = function() {
			if (config.goSwitch) {
				...
				config.index = config.index == config.imgDataLen ? 0 : config.index;
				...
				if(config.index == (config.imgDataLen-2) && !c){
	...
	**/
	...
	return {
		init: function(obj, e) {
			...
			config.imgDataLen = config.imgData;				// 设置图片数量
			config.index = config.imgDataLen-1;
			config.curImg = config.imgDataLen;
			   ...
			   dotList = $("focus_dot").getElementsByTagName('li');
			    ...
			    for(n = 0; n < dotList.length; n++ ){
						...
						dotList[n].onclick = function() {	
							if(config.curImg == this.index || config.curImg == this.index + config.imgDataLen) return;
							var n = 0;
							n = config.curImg > (config.imgDataLen - 1) ? 0 : config.curImg;
